...
const getUserConversations = async (userId) => {
  ...
  const conversationIds = convs.map((conv) => String(conv._id));
  let sentConversationIdSet = new Set();
  if (conversationIds.length > 0) {
    const sentConversationRows = await Message.aggregate([
      {
        $match: {
          senderId: userId,
    ...
    sentConversationIdSet = new Set(
      sentConversationRows.map((row) => String(row._id)),
    );
  }
  ...
  for (let conv of convs) {
    ...
    const convId = String(convObj.id || convObj._id);
    const sentByUserIds = new Set([...(convObj.sentByUserIds || [])]);
    if (sentConversationIdSet.has(convId)) {
      sentByUserIds.add(String(userId));
    }
    convObj.sentByUserIds = Array.from(sentByUserIds);
...
const updateLastMessage = async (conversationId, content, type, senderId) => {
  ...
  if (!conv.sentByUserIds) conv.sentByUserIds = [];
  if (!conv.sentByUserIds.includes(senderId)) {
    conv.sentByUserIds.push(senderId);
  }
